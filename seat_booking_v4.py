...
def automatic_book_seat_normal(name, num_seats):
    """
    Auto-booking function with prioritized seat selection:
    Priority 1: Fill rows from last to first (higher row numbers first)
    Priority 2: Prefer corner seats over center seats (to avoid isolation)
    Priority 3: Sort by ascending price (cheaper seats first)
    """
    for event in master_list:
        if event['name'] == name:
            seating = event['seating']
            ...
            # Calculate corner preference score for each seat
            # Corner seats get lower scores (higher priority)
            # Center seats get higher scores (lower priority)
            def corner_score(col, total_cols):
                """
                Calculate how much a seat is towards the center.
                Lower score = more towards corner (preferred)
                Higher score = more towards center (less preferred)
                """
                mid = total_cols / 2.0
                return abs(col - mid)
            ...
            # Create a list of tuples: (seat_coords, price, row_priority, corner_priority)
            seat_data = []
            for r, c in available_seats:
                price = calc_price(name, r, c, booking_date)
                row_priority = -r  # Negative so higher rows come first (last rows prioritized)
                corner_priority = corner_score(c, cols)
                seat_data.append(((r, c), price, row_priority, corner_priority))
            
            # Sort by multi-level priority:
            # 1. row_priority (ascending, which means higher rows first due to negative)
            # 2. corner_priority (ascending, which means corners first)
            # 3. price (ascending, which means cheaper first)
            seat_data.sort(key=lambda x: (x[2], x[3], x[1]))
            
            # Extract the top num_seats
            suggested_seats = [seat[0] for seat in seat_data[:num_seats]]
...
def automatic_book_seat_vip(name, num_seats):
    """
    Auto-booking function for VIP seats with prioritized seat selection:
    Priority 1: Fill rows from last to first (higher row numbers first)
    Priority 2: Prefer corner seats over center seats (to avoid isolation)
    Priority 3: Sort by ascending price (cheaper seats first)
    
    VIP Constraint: The selected seat and all surrounding seats (3x3 grid) must be unoccupied
    """
    for event in master_list:
        if event['name'] == name:
            seating = event['seating']
            ...
            # Collect all available VIP seats (center seat + all surrounding must be free)
            available_vip_seats = []
            for r in range(rows):
                for c in range(cols):
                    # Check if this seat can be a VIP seat
                    if seating[r][c] == 0:  # Center seat must be free
                        can_be_vip = True
                        # Check all surrounding seats in 3x3 grid
                        for dr in range(max(0, r-1), min(rows, r+2)):
                            for dc in range(max(0, c-1), min(cols, c+2)):
                                if seating[dr][dc] == 1:  # Any surrounding seat occupied
                                    can_be_vip = False
                                    break
                            if not can_be_vip:
                                break
                        ...
                        if can_be_vip:
                            available_vip_seats.append((r, c))
            
            # Check if enough VIP seats available
            if len(available_vip_seats) < num_seats:
                print(f"Only {len(available_vip_seats)} VIP seats available (need clear 3x3 area)")
                return
            ...
            # Create a list of tuples: (seat_coords, price, row_priority, corner_priority)
            seat_data = []
            for r, c in available_vip_seats:
                price = calc_price_vip(name, r, c, booking_date)
                row_priority = -r  # Negative so higher rows come first
                corner_priority = corner_score(c, cols)
                seat_data.append(((r, c), price, row_priority, corner_priority))
            ...
            print(f"Suggested VIP seats for '{name}': {suggested_seats}")
            confirm = input("book these VIP seats? (yes/no): ")
            
            if confirm.lower() == 'yes':
                total_price = 0
                for seat in suggested_seats:
                    r, c = seat
                    # Mark the center seat and all surrounding seats as booked
                    for dr in range(max(0, r-1), min(rows, r+2)):
                        for dc in range(max(0, c-1), min(cols, c+2)):
                            seating[dr][dc] = 1
                ...
                event['revenue'] += total_price
                print(f"VIP seats {suggested_seats} successfully booked for '{name}'. Total payment: {total_price:.2f}")
            else:
                print("Booking cancelled.")
            return
...
def view_seating_matrix(name):
    """
    Simple compact view of seating matrix.
    Shows: 0 for available, 1 for booked
    """
    for event in master_list:
        if event['name'] == name:
            seating = event['seating']
...
while choice !=6:
    ...
    if choice==1:
        ...
        if mode == "normal":
            ...
            automatic_book_seat_vip(name, num_seats)
        else:
            print("invalid mode")
        
    elif choice == 8:
        name = input("event name: ")
        view_seating_matrix(name)
